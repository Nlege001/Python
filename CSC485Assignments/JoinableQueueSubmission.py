...
class MyQueue(MyQueue0): #------class for thread saftey
    def __init__(self, capacity):
        self.lock = threading.Lock() #--lock for thread saftey
        self.jLock = threading.Lock() #-----lock for joining
...
class MyJoinableQueue(MyQueue): #------class for blocking 
    # your code here.
    def __init__(self, capacity):
        self.task = 0 # counter for controlling tasks that need to be done
    ...
    def put(self, value):
        self.putSem.acquire()
        super().put(value)
        self.task = self.task + 1 #--add new value to task
        self.getSem.release()
    ...
    def get(self):
        ...
        self.putSem.release()
        return result
        #we wont reduce the value of tasks as they might still be running
    
    def join(self):
        while self.task > 0: #---if there are tasks that are being run, block main thread until they're completed
            self.jLock.acquire()  
            self.jLock.release()
        
        
    def task_done(self):
        self.task = self.task - 1 # if the status of a task is done, decrement task by 1
...
blockingDemo() 
# Make sure it runs and gives output like:
